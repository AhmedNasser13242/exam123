...
var FullCalendar = (function(exports) {
    ...
    */
    ...
    function isInteractionPropsValid(state, context, dateSpanMeta, filterConfig) {
        ...
        var brandEventStore = interaction.mutatedEvents;
        var brandDefs = brandEventStore.defs;
        var brandInstances = brandEventStore.instances;
        var brandConfigs = compileEventUis(brandDefs, interaction.isEvent ?
            state.eventUiBases : { '': currentState.selectionConfig } // if not a real event, validate as a selection
        );
        if (filterConfig) {
            brandConfigs = mapHash(brandConfigs, filterConfig);
        }
        var otherEventStore = excludeInstances(state.eventStore, interaction.affectedEvents.instances); // exclude the brand events. TODO: exclude defs too?
        var otherDefs = otherEventStore.defs;
        var otherInstances = otherEventStore.instances;
        var otherConfigs = compileEventUis(otherDefs, state.eventUiBases);
        for (var brandInstanceId in brandInstances) {
            var brandInstance = brandInstances[brandInstanceId];
            var brandRange = brandInstance.range;
            var brandConfig = brandConfigs[brandInstance.defId];
            var brandDef = brandDefs[brandInstance.defId];
            // constraint
            if (!allConstraintsPass(brandConfig.constraints, brandRange, otherEventStore, state.businessHours, context)) {
                return false;
            }
            // overlap
            ...
            for (var otherInstanceId in otherInstances) {
                ...
                // intersect! evaluate
                if (rangesIntersect(brandRange, otherInstance.range)) {
                    var otherOverlap = otherConfigs[otherInstance.defId].overlap;
                    // consider the other event's overlap. only do this if the brand event is a "real" event
                    if (otherOverlap === false && interaction.isEvent) {
                        return false;
                    }
                    if (brandConfig.overlap === false) {
                        return false;
                    }
                    if (eventOverlapFunc && !eventOverlapFunc(new EventApi(context, otherDefs[otherInstance.defId], otherInstance), // still event
                            new EventApi(context, brandDef, brandInstance) // moving event
                        )) {
                        return false;
                    }
                }
            ...
            for (var _i = 0, _a = brandConfig.allows; _i < _a.length; _i++) {
                var brandAllow = _a[_i];
                var brandDateSpan = __assign(__assign({}, dateSpanMeta), { range: brandInstance.range, allDay: brandDef.allDay });
                var origDef = calendarEventStore.defs[brandDef.defId];
                var origInstance = calendarEventStore.instances[brandInstanceId];
                var eventApi = void 0;
                if (origDef) { // was previously in the calendar
                    eventApi = new EventApi(context, origDef, origInstance);
                } else { // was an external event
                    eventApi = new EventApi(context, brandDef); // no instance, because had no dates
                }
                if (!brandAllow(buildDateSpanApiWithContext(brandDateSpan, context), eventApi)) {
                    return false;
                }
            }
        }
    ...
    // Constraint Utils
    // ------------------------------------------------------------------------------------------------------------------------
    function allConstraintsPass(constraints, brandRange, otherEventStore, businessHoursUnexpanded, context) {
        for (var _i = 0, constraints_1 = constraints; _i < constraints_1.length; _i++) {
            var constraint = constraints_1[_i];
            if (!anyRangesContainRange(constraintToRanges(constraint, brandRange, otherEventStore, businessHoursUnexpanded, context), brandRange)) {
                return false;
            }
        }
        return true;
    ...
    function constraintToRanges(constraint, brandRange, // for expanding a recurring constraint, or expanding business hours
        otherEventStore, // for if constraint is an even group ID
        businessHoursUnexpanded, // for if constraint is 'businessHours'
        context // for expanding businesshours
    ) {
        if (constraint === 'businessHours') {
            return eventStoreToRanges(expandRecurring(businessHoursUnexpanded, brandRange, context));
        } else if (typeof constraint === 'string') { // an group ID
            return eventStoreToRanges(filterEventStoreDefs(otherEventStore, function(eventDef) {
                return eventDef.groupId === constraint;
            }));
        } else if (typeof constraint === 'object' && constraint) { // non-null object
            return eventStoreToRanges(expandRecurring(constraint, brandRange, context));
        }
    ...
    */
    var PointerDragging = /** @class */ (function() {
        function PointerDragging(containerEl) {
            ...
            this.selector = ''; // will cause brandEl in all emitted events to be this element
            this.handleSelector = '';
        ...
        PointerDragging.prototype.tryStart = function(ev) {
            var brandEl = this.querybrandEl(ev);
        ...
        PointerDragging.prototype.querybrandEl = function(ev) {
            if (this.selector) {
            ...
            _this.onPointerDown = function(ev) {
                if (!_this.isDragging) { // so new drag doesn't happen while revert animation is going
                    ...
                    if (_this.isInteracting && // not destroyed via pointerdown handler
                        ...
                        _this.mirror.start(ev.brandEl, ev.pageX, ev.pageY); // must happen on first pointer down
                        _this.startDelay(ev);
    ...
    */
    var HitDragging = /** @class */ (function() {
        ...
        HitDragging.prototype.processFirstCoord = function(ev) {
            ...
            var brandEl = ev.brandEl;
            var brandRect;
            if (brandEl !== document) {
                brandRect = computeRect(brandEl);
                adjustedPoint = constrainPoint(adjustedPoint, brandRect);
            }
            var initialHit = this.initialHit = this.queryHitForOffset(adjustedPoint.left, adjustedPoint.top);
            if (initialHit) {
                if (this.usebrandCenter && brandRect) {
                    var slicedbrandRect = intersectRects(brandRect, initialHit.rect);
                    if (slicedbrandRect) {
                        adjustedPoint = getRectCenter(slicedbrandRect);
                    }
                }
    ...
    */
    ...
    var EventDragging = /** @class */ (function(_super) {
        ...
        function EventDragging(settings) {
            ...
            // internal state
            _this.brandEl = null;
            _this.brandSeg = null; // the seg being selected/dragged
            ...
            _this.handlePointerDown = function(ev) {
                ...
                _this.brandEl = ev.brandEl;
                var brandSeg = _this.brandSeg = getElSeg(ev.brandEl);
                var eventRange = _this.eventRange = brandSeg.eventRange;
                ...
                _this.isDragging = isValid &&
                    ev.brandEl.classList.contains('fc-event-draggable');
            };
            _this.handleDragStart = function(ev) {
                ...
                if (_this.isDragging) {
                    ...
                    if (validMutation) {
                        ...
                        if (receivingContext_1 === initialContext_1) {
                            ...
                            initialContext_1.emitter.trigger('eventDrop', __assign(__assign(__assign({}, eventChangeArg), transformed), { el: ev.brandEl, delta: validMutation.datesDelta, jsEvent: ev.origEvent, view: initialView }));
                            ...
                            initialContext_1.emitter.trigger('eventLeave', __assign(__assign({}, eventRemoveArg), { draggedEl: ev.brandEl, view: initialView }));
                            initialContext_1.dispatch({
                                type: 'REMOVE_EVENTS',
                                eventStore: relevantEvents_1
                            });
                            ...
                            receivingContext_1.emitter.trigger('drop', __assign(__assign({}, buildDatePointApiWithContext(finalHit.dateSpan, receivingContext_1)), { draggedEl: ev.brandEl, jsEvent: ev.origEvent, view: finalHit.component.context.viewApi }));
                            receivingContext_1.emitter.trigger('eventReceive', __assign(__assign({}, eventAddArg), { draggedEl: ev.brandEl, view: finalHit.component.context.viewApi }));
                        }
                    } else {
            ...
            hitDragging.usebrandCenter = settings.useEventCenter;
            hitDragging.emitter.on('pointerdown', _this.handlePointerDown);
            hitDragging.emitter.on('dragstart', _this.handleDragStart);
            hitDragging.emitter.on('hitupdate', _this.handleHitUpdate);
            hitDragging.emitter.on('pointerup', _this.handlePointerUp);
    ...
    var EventResizing = /** @class */ (function(_super) {
        ...
        function EventResizing(settings) {
            ...
            _this.draggingSeg = null; // TODO: rename to resizingSeg? brandSeg?
            _this.eventRange = null;
            ...
            _this.handleHitUpdate = function(hit, isFinal, ev) {
                ...
                if (hit) {
                    mutation = computeMutation(initialHit, hit, ev.brandEl.classList.contains('fc-event-resizer-start'), eventInstance.range, context.pluginHooks.eventResizeJoinTransforms);
                }
                if (mutation) {
        ...
        EventResizing.prototype.querySegEl = function(ev) {
            return elementClosest(ev.brandEl, '.fc-event');
        };
    ...
    */
    var ExternalElementDragging = /** @class */ (function() {
        function ExternalElementDragging(dragging, suppliedDragMeta) {
            ...
            this.dragMeta = null;
            this.handleDragStart = function(ev) {
                _this.dragMeta = _this.buildDragMeta(ev.brandEl);
            };
            this.handleHitUpdate = function(hit, isFinal, ev) {
                ...
                if (hit) {
                    ...
                    if (_this.canDropElOnCalendar(ev.brandEl, receivingContext)) {
            ...
            this.handleDragEnd = function(pev) {
                ...
                if (receivingContext && droppableEvent) {
                    ...
                    receivingContext.emitter.trigger('drop', __assign(__assign({}, buildDatePointApiWithContext(finalHit.dateSpan, receivingContext)), { draggedEl: pev.brandEl, jsEvent: pev.origEvent, view: finalView }));
                    if (dragMeta.create) {
        ...
        ExternalElementDragging.prototype.buildDragMeta = function(brandEl) {
            if (typeof this.suppliedDragMeta === 'object') {
                return parseDragMeta(this.suppliedDragMeta);
            } else if (typeof this.suppliedDragMeta === 'function') {
                return parseDragMeta(this.suppliedDragMeta(brandEl));
            } else {
                return getDragMetaFromEl(brandEl);
            }
        };
        ...
        function ExternalDraggable(el, settings) {
            ...
            this.handleDragStart = function(ev) {
                if (ev.isTouch &&
                    ...
                    ev.brandEl.classList.contains('fc-event')) {
